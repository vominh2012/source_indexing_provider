...
int64_t ts_tree_cursor_goto_first_child_for_range(TSTreeCursor *_self, uint32_t start_byte_, uint32_t end_byte_) {
    TreeCursor *self = (TreeCursor *)_self;
    uint32_t initial_size = self->stack.size;
    uint32_t visible_child_index = 0;
    ...
    do {
        ...
        CursorChildIterator iterator = ts_tree_cursor_iterate_children(self);
        while (ts_tree_cursor_child_iterator_next(&iterator, &entry, &visible)) {
            uint32_t start_byte = entry.position.bytes;
            uint32_t end_byte = entry.position.bytes + ts_subtree_size(*entry.subtree).bytes;
            bool at_goal = (end_byte == end_byte_) && (start_byte == start_byte_);
            uint32_t visible_child_count = ts_subtree_visible_child_count(*entry.subtree);
